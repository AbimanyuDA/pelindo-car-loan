...
export default function AdminSchedulePage() {
  ...
  const retryMutation = useMutation({
    mutationFn: (loanRequestId: number) =>
      scheduleService.retryScheduling(loanRequestId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["all-schedules"] });
      queryClient.invalidateQueries({ queryKey: ["waiting-resources"] });
    },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <Card
          className={stats.waiting > 0 ? "bg-amber-50 border-amber-200" : ""}
        >
          <CardContent className="py-4">
      ...
      {stats.waiting > 0 && (
        <Alert variant="warning" title="Perhatian">
          Terdapat {stats.waiting} jadwal yang menunggu kendaraan atau driver
          tersedia. Silakan assign secara manual atau tunggu hingga resource
          tersedia.
        </Alert>
      )}
